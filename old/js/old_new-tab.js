...
var originalSideOverlay =
  document.getElementsByClassName('side-container')[0].innerHTML;
var data = null;
var defaultData = {
...
function initCards() {
  ...
  for (var i = 0; i < cardKeys.length; i++) {
    ...
    html +=
      ...
      data['cards'][cardKeys[i]]['url'] +
      '>';
    html +=
      "<div class='shortcut-card waves-effect waves-blue' draggable='true'>";
    html +=
      "<img class='shortcut-icon' alt='" +
      data['cards'][cardKeys[i]]['name'] +
      " shortcut' src='" +
      data['cards'][cardKeys[i]]['icon'] +
      "' draggable='false' />";
    html +=
...
function initCardOptions() {
  ...
  function cardOpacity() {
    var css =
      '.fade-in-card{-webkit-animation: fade-in-card .3s cubic-bezier(.39, .575, .565, 1) both;animation: fade-in-card .3s cubic-bezier(.39, .575, .565, ' +
      data['card-opacity'] +
      ') both}@-webkit-keyframes fade-in-card {0% {opacity: 0}100% {opacity: ' +
      data['card-opacity'] +
      '}}@keyframes fade-in-card {0% {opacity: 0}100% {opacity: ' +
      data['card-opacity'] +
      '}}';
...
function updateData(action) {
  ...
  for (var i = 0; i < cardList.length; i++) {
    ...
    data['cards'][cardKeys[i]]['icon'] =
      cardList[i].childNodes[0].childNodes[0].childNodes[0].src;
    data['cards'][cardKeys[i]]['name'] =
      cardList[i].childNodes[0].childNodes[0].childNodes[1].innerHTML;
  }
...
function refreshCard(index) {
  ...
  card.childNodes[0].childNodes[0].childNodes[0].src =
    data['cards'][index - 1]['icon'];
  card.childNodes[0].childNodes[0].childNodes[1].innerHTML =
    data['cards'][index - 1]['name'];
}
...
function getWeather(lat, lon) {
  var url =
    ...
    '&lon=' +
    lon +
    '&units=imperial&APPID=9fe397f7ea57307ce1add4c7660c3a5d';
...
function showWeather(weatherData) {
  ...
  if (data['header-weather-units'] === 'Fahrenheit') {
    ...
    weather.innerHTML =
      Math.round((5 / 9) * (weatherData.main.temp - 32)) + '°C';
    floatWeather.innerHTML =
      Math.round((5 / 9) * (weatherData.main.temp - 32)) + '°C';
  }
